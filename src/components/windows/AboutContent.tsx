import { motion } from 'framer-motion';
import { useSettings } from '../../context/SettingsContext';

const AboutContent = () => {
  const { isDarkMode } = useSettings();

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className={`space-y-6 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}
    >
      <div className="flex items-center gap-6 flex-wrap">
        <motion.div
          className="w-32 h-32 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-4xl font-bold shadow-2xl text-white"
          whileHover={{ scale: 1.05, rotate: 5 }}
          transition={{ type: "spring", stiffness: 300 }}
        >
          YS
        </motion.div>
        <div>
          <h1 className="text-4xl font-bold mb-2">Yuriy Stenin</h1>
          <p className={`text-xl ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Full Stack Developer</p>
          <p className={`mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Building elegant solutions for complex problems</p>
        </div>
      </div>

      <div className="space-y-4">
        <h2 className="text-2xl font-semibold">Who I Am</h2>
        <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} leading-relaxed`}>
          I have a proven track record of delivering high-quality web solutions. My expertise lies in
          PHP, Drupal, JavaScript, MySQL, Git, and I am committed to upholding Clean Code principles
          (DDD, SOLID, KISS, DRY) to ensure the maintainability and scalability of projects.
        </p>
        <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} leading-relaxed`}>
          I hold a Bachelor's degree in Electrical, Electronics, and Communications Engineering from
          Kharkiv Polytechnics Institute. Additionally, I have earned a second master's degree in
          Cybersecurity. Throughout my career, I have successfully led and contributed to a range of
          diverse projects.
        </p>
      </div>

      <div className="space-y-4">
        <h2 className="text-2xl font-semibold">Skills & Expertise</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SkillCard skill="PHP & Drupal" level={95} color="from-orange-500 to-red-500" isDarkMode={isDarkMode} />
          <SkillCard skill="JavaScript" level={90} color="from-yellow-500 to-orange-500" isDarkMode={isDarkMode} />
          <SkillCard skill="Python (Django, Flask)" level={85} color="from-blue-500 to-cyan-500" isDarkMode={isDarkMode} />
          <SkillCard skill="MySQL & Database" level={88} color="from-green-500 to-emerald-500" isDarkMode={isDarkMode} />
          <SkillCard skill="Git & GitHub" level={92} color="from-purple-500 to-pink-500" isDarkMode={isDarkMode} />
          <SkillCard skill="Clean Code Principles" level={90} color="from-indigo-500 to-blue-500" isDarkMode={isDarkMode} />
        </div>
      </div>

      <div className="space-y-4">
        <h2 className="text-2xl font-semibold">Technologies</h2>
        <div className="flex flex-wrap gap-2">
          {['PHP', 'Drupal 7/8/9', 'JavaScript', 'Python', 'Django', 'Flask',
            'MySQL', 'Git', 'GitHub', 'REST APIs', 'jQuery', 'Symfony',
            'HTML5', 'CSS3', 'Bootstrap', 'DDD', 'SOLID', 'KISS', 'DRY'].map((tech) => (
            <motion.span
              key={tech}
              className={`px-4 py-2 rounded-full text-sm border
                ${isDarkMode
                  ? 'bg-white/10 border-white/20'
                  : 'bg-black/5 border-black/10 text-gray-800'}`}
              whileHover={{
                scale: 1.05,
                backgroundColor: isDarkMode ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.1)',
                borderColor: isDarkMode ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.2)'
              }}
            >
              {tech}
            </motion.span>
          ))}
        </div>
      </div>

      {/* Download CV Button */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.8 }}
        className={`mt-8 p-6 rounded-xl border bg-gradient-to-r
          ${isDarkMode
            ? 'from-purple-500/20 to-blue-500/20 border-white/20'
            : 'from-purple-500/10 to-blue-500/10 border-black/10'}`}
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h3 className="text-xl font-semibold mb-2">📝 Download My Resume</h3>
            <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Get a detailed PDF of my experience, skills, and education
            </p>
          </div>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => {
              // For now, open LinkedIn profile
              // In production, replace with actual PDF download
              const confirmed = window.confirm(
                '📄 CV Download\n\n' +
                'For the full CV experience, please visit my LinkedIn profile.\n' +
                'Would you like to open it now?\n\n' +
                'Note: You can also add a direct PDF link here later!'
              );
              if (confirmed) {
                window.open('https://www.linkedin.com/in/yuriy-stenin/', '_blank');
              }
            }}
            className="px-6 py-3 bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 rounded-lg font-medium shadow-lg hover:shadow-purple-500/50 transition-all text-white"
          >
            💾 Download CV (PDF)
          </motion.button>
        </div>
      </motion.div>
    </motion.div>
  );
};

interface SkillCardProps {
  skill: string;
  level: number;
  color: string;
  isDarkMode: boolean;
}

const SkillCard = ({ skill, level, color, isDarkMode }: SkillCardProps) => (
  <motion.div
    className={`p-4 rounded-xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/5'}`}
    whileHover={{ scale: 1.02, backgroundColor: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}
    transition={{ type: "spring", stiffness: 300 }}
  >
    <div className="flex justify-between items-center mb-2">
      <p className="font-medium">{skill}</p>
      <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{level}%</span>
    </div>
    <div className={`h-2 rounded-full overflow-hidden ${isDarkMode ? 'bg-white/10' : 'bg-black/10'}`}>
      <motion.div
        className={`h-full bg-gradient-to-r ${color}`}
        initial={{ width: 0 }}
        animate={{ width: `${level}%` }}
        transition={{ duration: 1, delay: 0.2, ease: "easeOut" }}
      />
    </div>
  </motion.div>
);

export default AboutContent;
